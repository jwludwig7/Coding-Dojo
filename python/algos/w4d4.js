...
function binarySearch(sortedNums, num) {
    ...
    if (sortedNums.length == 1 && sortedNums[0] != num) {
        return false;
    }
    if (num == sortedNums[mid]) {
        return true;
    } else if (num < sortedNums[mid]) {
        return binarySearch(sortedNums.slice(0, mid), num);
    } else if (num > sortedNums[mid]) {
        return binarySearch(sortedNums.slice(mid), num);
    }
}

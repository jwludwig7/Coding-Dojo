...
//     let middIdx = Math.floor((left + right) / 2);
//     [nums[middIdx], nums[right]] = [nums[right], nums[middIdx]];
//     let tempPivotIdx = right;
//     right --;
//     while(left <= right){
//         if(nums[left] < nums[tempPivotIdx]){
...
function partition2(nums = [], left = 0, right = nums.length - 1) {
    let pivot = nums[Math.floor((right + left) / 2)]; //middle element
...
console.log(partition2(nums2));
console.log(partition2(nums3));
console.log(partition2(nums4));
